...
```php
getProjectSchema($xOmnismithProjectId): \Omnismith\Sdk\Model\ProjectSchemaResponse
```
...
try {
    $result = $apiInstance->getProjectSchema($xOmnismithProjectId);
    print_r($result);
} catch (Exception $e) {
...
```php
getUiLayout($xOmnismithProjectId): \Omnismith\Sdk\Model\UiLayoutResponse
```
...
try {
    $result = $apiInstance->getUiLayout($xOmnismithProjectId);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling SchemaApi->getUiLayout: ', $e->getMessage(), PHP_EOL;
}
```
...
[**\Omnismith\Sdk\Model\UiLayoutResponse**](../Model/UiLayoutResponse.md)
